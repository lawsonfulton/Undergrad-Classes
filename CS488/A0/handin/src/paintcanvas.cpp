...
void PaintRect::draw(QPainter& painter) {
    ...
    painter.fillRect(QRect(x, y, width, height), m_colour);
}
...
void PaintCanvas::set_colour(Colour colour) {
    switch (colour) {
        case BLACK:
            m_colour = QColor(0, 0, 0);
            break;
        case RED:
            m_colour = QColor(255, 0, 0);
            break;
        case GREEN:
            m_colour = QColor(0, 255, 0);
            break;
        case BLUE:
            m_colour = QColor(0, 0, 255);
            break;
        default:
            break;
    }
...
void PaintCanvas::clear() {
    //Iterate over stored shapes and delete them
...
void PaintCanvas::mouseReleaseEvent ( QMouseEvent * event ) {
    ...
    if (event->button() == Qt::LeftButton) { // Left mouse button
        ...
        switch (m_mode) {
        case DRAW_LINE:
            shape = new PaintLine( m_start_pos, to, m_colour );
            break;
        case DRAW_OVAL:
            shape = new PaintOval( m_start_pos, to, m_colour );
            break;
        case DRAW_RECTANGLE:
            shape = new PaintRect( m_start_pos, to, m_colour );
            break;
        default:
            break;
        }
